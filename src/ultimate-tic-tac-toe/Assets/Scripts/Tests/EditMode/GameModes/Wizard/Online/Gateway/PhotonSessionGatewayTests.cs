...
namespace Tests.EditMode.GameModes.Wizard.Online.Gateway
{
    [TestFixture]
    [Category("Unit")]
    public class PhotonSessionGatewayTests
    {
        ...
        public async Task WhenJoinRandomOrCreateThrows_ThenGatewayWrapsIntoPhotonSessionTransportException()
        {
            ...
            var sut = new PhotonSessionGateway(transport);
            var options = new MatchmakingRoomOptions("eu", "classic", "HASH1234");
        ...
        private sealed class FakeTransport : IPhotonSessionTransport
        {
            ...
            public UniTask JoinSessionAsync(SessionId sessionId, string region, string currentUserId) => 
                JoinException != null ? throw JoinException : UniTask.CompletedTask;
            ...
            public UniTask<PhotonTransportMatchmakingResult> JoinRandomOrCreateSessionAsync(MatchmakingRoomOptions options, CancellationToken ct)
            {
                ...
                return JoinRandomOrCreateException != null ? throw JoinRandomOrCreateException : UniTask.FromResult(JoinRandomOrCreateResult);
            }

            public UniTask LeaveSessionAsync() => LeaveSession?.Invoke() ?? UniTask.CompletedTask;
