...
namespace Tests.EditMode.GameModes.Wizard.Matchmaking
{
    ...
    public class MatchmakingFsmCancelProtocolTests
    {
        ...
        public async Task WhenRequestCancelCalledDuringSearching_ThenTransitionsThroughCancelPendingToCancelled()
        {
            ...
            {
                EnterQueue = (_, _) => UniTask.FromResult(new QueueEntry("room-1", immediateResult: null)),
        ...
        public async Task WhenNotifySessionStartFailedAfterFound_ThenTransitionsToTerminalModal()
        {
            ...
            var service = new ControlledMatchmakingService
            {
                ...
                WaitForMatch = (_, _) => UniTask.FromException<MatchmakingResult>(new InvalidOperationException("Not expected")),
                Leave = _ => UniTask.CompletedTask,
            };
        ...
        public async Task WhenSearchSuccessArrivesFromPreviousEpoch_ThenIgnoredAndCurrentStatePreserved()
        {
            ...
            var service = new ControlledMatchmakingService
            {
                EnterQueue = (_, _) => UniTask.FromResult(new QueueEntry("room", immediateResult: null)),
                WaitForMatch = (_, _) =>
                {
        ...
        public async Task WhenServiceThrowsConnectionLostException_ThenTransitionsToTerminalModalNotFailed()
        {
            ...
            {
                EnterQueue = (_, _) => UniTask.FromResult(new QueueEntry("room", immediateResult: null)),
                WaitForMatch = (_, _) => UniTask.FromException<MatchmakingResult>(new ConnectionLostException("lost")),
                Leave = _ => UniTask.CompletedTask,
            };
        ...
        public async Task WhenServiceEnterQueueReturnsNull_ThenTryStartSearchReturnsFalseAndStateIsFailed()
        {
            ...
            var service = new ControlledMatchmakingService
            {
                EnterQueue = (_, _) => UniTask.FromResult<QueueEntry>(null),
                WaitForMatch = (_, _) => UniTask.FromException<MatchmakingResult>(new InvalidOperationException("Not expected")),
                Leave = _ => UniTask.CompletedTask,
            };
        ...
        public async Task WhenWaitForMatchAsyncReturnsNull_ThenTransitionsToFailed()
        {
            ...
            {
                EnterQueue = (_, _) => UniTask.FromResult(new QueueEntry("room", immediateResult: null)),
                WaitForMatch = (_, _) => UniTask.FromResult<MatchmakingResult>(null),
                Leave = _ => UniTask.CompletedTask,
            };
        ...
        private sealed class ControlledMatchmakingService : IMatchmakingService
        {
            ...
            public UniTask<QueueEntry> EnterQueueAsync(MatchmakingRequest request, CancellationToken ct) => 
                EnterQueue?.Invoke(request, ct) ?? UniTask.FromException<QueueEntry>(new InvalidOperationException("EnterQueue is not configured."));

            public UniTask<MatchmakingResult> WaitForMatchAsync(QueueEntry entry, CancellationToken ct) => WaitForMatch?.Invoke(entry, ct) ?? UniTask.FromException<MatchmakingResult>(new InvalidOperationException("WaitForMatch is not configured."));

            public UniTask LeaveAsync(CancellationToken ct) => Leave?.Invoke(ct) ?? UniTask.CompletedTask;
        }
